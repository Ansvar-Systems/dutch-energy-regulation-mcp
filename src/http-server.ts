...
const TOOLS = [
  {
    ...
    description:
      "Search across Dutch energy regulations from ACM, RVO, and SodM. Covers Elektriciteitswet 1998, Gaswet, Warmtewet, and the new Energiewet.",
    inputSchema: {
  ...
  },
  {
    ...
    description: "Search TenneT NL grid codes, congestion management rules, and network connection requirements.",
    inputSchema: {
      type: "object" as const,
      properties: {
  ...
  },
  {
    ...
    description: "Search ACM method decisions, tariff determinations, and market monitoring reports.",
    inputSchema: {
      type: "object" as const,
      properties: {
        query: { type: "string", description: "Search query" },
...
const SearchRegulationsArgs = z.object({
  ...
  regulator: z.enum(["acm", "rvo", "sodm"]).optional(),
  type: z.enum(["wet", "besluit", "regeling", "beleidsregel"]).optional(),
...
const SearchGridCodesArgs = z.object({
  query: z.string().min(1),
  code_type: z.enum(["technical_regulation", "market_regulation", "grid_connection", "balancing", "congestion_management"]).optional(),
  limit: z.number().int().positive().max(100).optional(),
});
      ...
      switch (name) {
        ...
        case "nl_energy_about": {
          ...
          return textContent({
            ...
            version: pkgVersion,
            description:
              "Dutch energy regulation MCP. Covers ACM, TenneT NL, RVO, SodM.",
