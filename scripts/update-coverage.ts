 ...
 *
 * Reads the nl-energy.db, queries each table for row counts,
...
const DB_PATH = process.env["NL_ENERGY_DB_PATH"] ?? join(__dirname, "..", "data", "nl-energy.db");
...
const SOURCE_QUERIES: Record<string, string> = {
  acm: "SELECT COUNT(*) as c FROM regulations WHERE regulator_id = 'acm'",
  rvo: "SELECT COUNT(*) as c FROM regulations WHERE regulator_id = 'rvo'",
  sodm: "SELECT COUNT(*) as c FROM regulations WHERE regulator_id = 'sodm'",
  tennet: "SELECT COUNT(*) as c FROM grid_codes",
};
